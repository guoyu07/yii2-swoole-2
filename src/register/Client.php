...
namespace xutl\swoole\register;

use xutl\swoole\Server;
use xutl\swoole\clusters;
use xutl\swoole\clusters\Host;

/**
 * 注册服务器客户端
 *
 * @package xutl\swoole\register
 */
class Client
{
    /**
    ...
    /**
     * @var Rpc|\xutl\swoole\rpc\Client
     */
    ...
    public static function onConnect()
    {
        try
        {
            ...
            if ($rs)
            {
                ...
                \xutl\swoole\Server::$instance->info('register clusters host group: '. self::$host->group .'#'. self::$host->id .'(' . self::$host->ip . ':' . self::$host->port . ') success.');
            }
            else
            {
        ...
        {
            \xutl\swoole\Server::$instance->warn('register clusters host('. self::$host->ip . ':' . self::$host->port .') fail. '. $e->getMessage());
        }
    }
