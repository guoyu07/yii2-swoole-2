...
namespace xutl\swoole\Clusters;
 ...
 * 任务服务器
 *
 * @package xutl\swoole\TaskServer
 */
class TaskServer
{
    /**
    ...
    public function start($ip, $port)
    {
        ...
        $config = [
            ...
            'open_eof_check'     => true,
            'open_eof_split'     => true,
            'package_eof'        => \xutl\swoole\rpc\Server::$EOF,
        ];
    ...
    public function onStart()
    {
        if ($this->server->worker_id === 0)
        {
            ...
            \xutl\swoole\register\Client::init(Server::$instance->config['clusters']['group'] ?: 'default', $id, true);
        }
        ...
        if (!class_exists($className))
        {
            ...
            $className = '\\xutl\\swoole\\WorkerTask';
        }
